...
export function formatTime(time, format) {
  ...
  if (diff > 3600 * 24 * 7 || format) {
    // 一个月以上或传递了格式，采用标准日期格式化方式处理
...
export function deepMergeLeft(target, source) {
  ...
  Object.keys(target).forEach(property => {
    if (
      Object.prototype.toString.call(target[property]) === "[object Object]"
    ) {
      target[property] = deepMergeLeft(target[property], source[property]);
    } else if (
      Object.prototype.toString.call(source[property]) === "[object Object]"
    ) {
      target[property] = deepMerge({}, source[property]);
    } else if (
      Object.prototype.toString.call(source[property]) === "[object Array]"
    ) {
