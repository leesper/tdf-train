...
export default {
  ...
  queryAllRoleMenus: config => {
    ...
    return {
      code: 1,
      message: "操作成功",
      data: roleMenus.filter(item => {
        return params.id === item.roleId;
      })
    };
  },
  /**
  ...
  queryAllRoleUsers: config => {
    ...
    const params = param2Obj(config.url);
    if (
      userRoles.findIndex(item => {
        return item.roleId === params.id;
      }) === -1
    ) {
      // 生成几个user
      for (let i = 0; i < 20; i++) {
    ...
    const userRolesResult = userRoles.filter(item => {
      return item.roleId === params.id;
    });
    return {
      code: 1,
      ...
      data: users.filter(user => {
        return (
          userRolesResult.findIndex(userRole => {
